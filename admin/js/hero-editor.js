...
function validateFile(file, allowedTypes) {
  ...
  if (!allowedTypes.includes(file.type)) {
    throw new Error(
      `Formato não permitido para "${file.name}".`
    );
  }

  if (file.size > MAX_FILE_SIZE) {
    throw new Error(
      `"${file.name}" deve ter no máximo 25 MB.`
    );
  }
}

/* =========================================================
   COMPONENTE VISUAL DE MÍDIA
========================================================= */
...
function createMediaEditor({
  ...
  function calculateBaseImageSize() {
    ...
    /*
     * Cover:
     * cobre todo o quadro sem deixar áreas vazias.
     */
  ...
  function setSource(
    ...
    state.offsetX = toNumber(offsetX, 0);
    state.offsetY = toNumber(offsetY, 0);
    ...
    image.onerror = () => {
      console.warn(
        `Não foi possível carregar a mídia "${name}".`
      );
  ...
  function bind() {
    if (
      ...
      console.warn(
        `Editor de mídia incompleto: ${name}`
      );
...
function updateLivePreview() {
  const container =
    previewElements.live;

  if (!container || !mainEditor) {
    return;
  }

  const source =
    mainEditor.getSource();
...
function bindGeneralPreview() {
  ...
  fields.enabled?.addEventListener(
    'change',
    updateInformationPreview
  );
}
...
async function loadNextDisplayOrder() {
  ...
  if (error) {
    console.warn(
      'Não foi possível calcular a ordem:',
      error
    );
...
function populateHero(hero) {
  ...
  mainEditor.setSource(
    ...
    ),
    {
      ...
      offsetX:
        hero.image_offset_x ?? 0,

      offsetY:
        hero.image_offset_y ?? 0
    }
  );

  cardEditor.setSource(
    getPublicUrl(
      hero.card_image_path
    ),
    {
      scale:
        hero.card_image_scale ?? 1,

      offsetX:
        hero.card_image_offset_x ?? 0,

      offsetY:
        hero.card_image_offset_y ?? 0
    }
  );
...
async function validateSlugAvailability(
  ...
  if (data?.length) {
    throw new Error(
      `Já existe outro herói usando o identificador "${slug}".`
    );
  }
}
...
async function saveHero(event) {
  ...
  try {
    ...
    await validateSlugAvailability(
      slug
    );
    ...
    if (!heroId) {
      setTimeout(() => {
        location.href =
          `./hero-editor.html?id=${savedHero.id}&tab=general`;
      }, 700);
