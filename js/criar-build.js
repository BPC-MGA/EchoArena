...
import { carregarDadosAnalise, analisarBuild, renderAnalise } from './build-analise.js?v=12';
...
function resumoInteligenteDoItem(item, stats, pieces, bonuses) {
  const atributos = Object.entries(stats || {}).slice(0, 2);
  if (!item.set) {
    return `<div class="hud-kind">BÔNUS DO EQUIPAMENTO</div>
      <div class="hud-status">${atributos.length
        ? atributos.map(([key, value]) => `<span class="up">${esc(formatStatValue(key, value))} <i>${esc(formatStatLabel(key))}</i></span>`).join('')
        : '<span class="neutral">Sem atributos nesta raridade</span>'}</div>`;
  }

  const ordenados = [...bonuses].sort((a, b) => Number(a.required_pieces || 0) - Number(b.required_pieces || 0));
  const ativos = ordenados.filter(b => pieces >= Number(b.required_pieces || 0));
  const proximo = ordenados.find(b => pieces < Number(b.required_pieces || 0));
  const faltam = proximo ? Math.max(0, Number(proximo.required_pieces) - pieces) : 0;

  return `<div class="hud-kind">${esc(item.set.nome)}</div>
    <div class="hud-setline"><strong>${pieces} peça${pieces === 1 ? '' : 's'}</strong><span>${ativos.length} bônus ativo${ativos.length === 1 ? '' : 's'}</span></div>
    <div class="hud-meter"><i style="width:${Math.min(100, pieces / Math.max(1, ...ordenados.map(b => Number(b.required_pieces || 0))) * 100)}%"></i></div>
    <div class="hud-status">${ativos.length
      ? `<span class="up">✓ ${esc(ativos.at(-1).title || `Bônus de ${ativos.at(-1).required_pieces} peças`)}</span>`
      : '<span class="neutral">Nenhum bônus de conjunto ativo</span>'}
      ${proximo ? `<span class="next">Faltam ${faltam} · próximo: ${esc(proximo.title || `${proximo.required_pieces} peças`)}</span>` : '<span class="up">Conjunto completo</span>'}</div>`;
}
...
function renderDetalheEquipamento(item = equipamentoSelecionado) {
  ...
  if (floating) {
    floating.style.setProperty('--rc', cor);
    floating.innerHTML = `<span class="equipment-float-icon">◇</span><div class="equipment-hud"><strong>${esc(item.nome)}</strong><small>${esc(slotDoItem?.label || item.slotLabel || 'Equipamento')} · ${esc(nomeDoNivel(item))}</small>${resumoInteligenteDoItem(item, stats, pieces, bonuses)}</div>`;
  }
  ...
  detail.querySelectorAll('[data-rarity]').forEach(button => {
    button.onclick = () => {
      ...
      if (window.matchMedia('(max-width:720px)').matches) fecharPop();
    };
  });
}
  ...
  grid.innerHTML = linhas.slice(0, 5).map(linha => {
    ...
    return `<article class="real-stat-card ${negativo ? 'negative' : ''} ${mudou ? 'changed' : 'unchanged'}" style="--stat-color:${esc(linha.cor)}">
      ...
      <div class="real-stat-current">
        <small>${mudou ? 'CÁLCULO COM EQUIPAMENTOS' : 'VALOR ATUAL'}</small>
        ${mudou ? `<div class="stat-equation">
          <span><i>BASE</i><b>${formatImpactValue(linha, linha.base)}</b></span>
          <strong class="${negativo ? 'penalty' : ''}">${linha.difference >= 0 ? '+' : '−'} ${formatImpactDelta(linha, Math.abs(linha.difference))}</strong>
          <em>=</em>
          <span class="result"><i>ATUAL</i><b>${formatImpactValue(linha, linha.valor)}</b></span>
        </div>` : `<div><b>${formatImpactValue(linha, linha.valor)}</b></div>`}
      </div>
